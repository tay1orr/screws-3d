...
export const TUTORIAL_LEVEL = {
  ...
  rules: { activeBoxCount: 2, boxCapacity: 3, bufferCapacity: 5 },
...
const windmillColors = ['red', 'blue', 'green', 'yellow', 'purple'];
...
for (const [z, angle] of [[0.42, Math.PI / 5.4], [-0.42, -Math.PI / 5.4]]) {
  windmillPieces.push(piece({
    ...
    [-0.72, 0.10, 0.14], [0.72, 0.10, 0.14],
  ], [0, 1, 0], windmillCursor)));
}

const hubY = 1.48;
const bladeZ = 1.12;
for (const [bladeIndex, angle] of [0, Math.PI / 2, Math.PI, Math.PI * 1.5].entries()) {
  const radius = 0.92;
  windmillPieces.push(piece({
    id: `windmill-blade-${bladeIndex}`,
    size: [0.34, 1.72, 0.12],
    ...
    [0, -0.48, 0.07], [0, 0.48, 0.07],
  ], [0, 0, 1], windmillCursor)));
}

windmillPieces.push(piece({
  id: 'windmill-hub',
  blockedBy: ['windmill-blade-0', 'windmill-blade-1', 'windmill-blade-2', 'windmill-blade-3'],
  size: [0.52, 0.52, 0.20],
  pos: [0, hubY, 1.25],
  rot: [0, 0, Math.PI / 4],
  color: PALETTE.roofDark,
}, cycleScrews(windmillColors, [
  [-0.13, 0.10, 0.11], [0.13, -0.10, 0.11],
], [0, 0, 1], windmillCursor)));
...
export const WINDMILL_LEVEL = {
  ...
  rules: { activeBoxCount: 3, boxCapacity: 3, bufferCapacity: 5 },
  binQueue: [
